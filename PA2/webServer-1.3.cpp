...
void *connection_handler(void *socket_desc)
{
  ...
  printf("opening sock: %d\n", sock);
  while ( keepalive && ( read_size = recv(sock, client_message, MSG_SIZE, 0 ) > 0 ) ) {
    ...
    printf("Connection: %s\n", request.headers[ "Connection" ].c_str() );
    keepalive = request.headers[ "Connection" ].compare( "keepalive" ) == 0
      || request.headers[ "Connection" ].compare( "Keepalive" ) == 0
      || request.headers[ "Connection" ].compare( "KeepAlive" ) == 0
      || request.headers[ "Connection" ].compare( "keep-alive" ) == 0
      || request.headers[ "Connection" ].compare( "Keep-alive" ) == 0
      || request.headers[ "Connection" ].compare( "Keep-Alive" ) == 0;

    if ( keepalive ) {
      printf("keeping it alive\n");
    }
